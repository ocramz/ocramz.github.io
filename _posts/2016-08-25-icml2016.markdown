...
Small reminder about the operators: `(<$>) :: Functor f => (a -> b) -> f a -> f b` is a synonym for `fmap`, whereas `(*>) :: Applicative f => f a -> f b -> f b` and `(<*) :: Applicative f => f a -> f b -> f a` can be read as "sequence", i.e. they perform both actions that appear as their arguments and retain the second and first results, respectively.
