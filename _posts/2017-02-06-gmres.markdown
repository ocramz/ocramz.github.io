...
Since the columns of the $$Q_i$$ matrices are orthogonal, the action of the Hermitian conjugate of $$Q_i$$ on a vector is equal to that of its transpose: $$Q_i^{-1} v = Q_i^\dagger v$$; this lets us rewrite the right-hand side residual above as follows:
...
At this point we focus on the Hessenberg matrix $$H_{i+1}$$: this is a $$m \times i + 1$$ matrix in which all the elements below the first sub-diagonal are identically zero. This is convenient from a computational point of view, as we will see shortly.
